...
export async function GET(req: NextRequest) {
  ...
  const access = await requireOsintAccess(req, "crypto-risk");
  ...
  try {
    ...
    const message =
      err instanceof Error ? err.message : "Risk check failed";

    if (isTimeoutLike(err) || isSoftProviderFailure(err)) {
      return NextResponse.json(
        {
          error:
            "Risk providers are slow or unavailable. Try again shortly.",
        },
        { status: 502 },
      );
    }
