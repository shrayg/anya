...
function verifySquareSignature(
  ...
  const hmac = createHmac("sha256", signatureKey).update(payload).digest("base64");
...
export async function POST(request: NextRequest) {
  ...
  try {
      ...
      if (payment?.status === "COMPLETED") {
        ...
        if (meta) {
          const pending = meta.paymentId
            ? await prisma.payment.findUnique({ where: { id: Number(meta.paymentId) } })
            : null;

          await fulfillBillingPayment({
            meta,
            checkoutSessionId:
              pending?.stripeSessionId || payment.orderId || payment.id || "square",
    ...
    if (event.type === "order.updated") {
      ...
      if (order?.id && (order.state === "OPEN" || order.state === "COMPLETED")) {
