...
function statusDotClass(status: PublicStatusLevel) {
  if (status === "operational") return "bg-emerald-400 shadow-[0_0_12px_rgba(52,211,153,0.55)]";
  if (status === "degraded") return "bg-amber-400 shadow-[0_0_12px_rgba(251,191,36,0.5)]";
...
export function StatusPageContent() {
  ...
  return (
    <section className="relative z-20 mx-auto w-full max-w-3xl px-2 pb-20 pt-6 md:pt-10">
      <div
        aria-hidden
      ...
      <header className="relative mb-10 space-y-4">
        ...
        <h1 className="font-[family-name:var(--font-bruno-ace-sc)] text-3xl tracking-wide text-white md:text-4xl">
          Platform status
        </h1>
        <p className="max-w-2xl text-sm leading-6 text-zinc-400 md:text-base">
          Live health for {siteConfig.name} core services. Checks refresh about
          every minute.
        </p>
      </header>
      ...
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            />
            <div>
              <p className={clsx("text-lg font-semibold", statusTextClass(overall))}>
                {loading && !data ? "Checking systems…" : OVERALL_COPY[overall]}
              </p>
          ...
          >
            <RefreshCw
              className={clsx("size-3.5", (loading || refreshing) && "animate-spin")}
            />
            Refresh
          </button>
        </div>
        ...
        {data ? (
          <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 border-t border-white/10 pt-4 text-xs text-zinc-500">
            <span>
              <span className="text-emerald-300">{data.summary.operational}</span>{" "}
              operational
            </span>
            <span>
            ...
            <span>
              <span className="text-rose-300">{data.summary.outage}</span> outage
            </span>
          </div>
        ) : null}
      </div>
