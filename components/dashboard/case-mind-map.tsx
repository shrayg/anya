...
import { buildCaseMindMap, type CaseWithSearches, type MindMapNodeData } from "@/lib/case-mind-map";
...
function IntelNode({ data }: NodeProps<Node<MindMapNodeData>>) {
  ...
  return (
    ...
    >
      <Handle className="!border-violet-400 !bg-violet-300" position={Position.Top} type="target" />
      <p
      ...
      <p className="mt-1 text-sm font-semibold leading-snug text-white">{data.label}</p>
      {data.sublabel && (
        <p className="mt-1 break-all text-xs leading-relaxed text-zinc-400">{data.sublabel}</p>
      )}
      <Handle className="!border-teal-400 !bg-teal-300" position={Position.Bottom} type="source" />
    </div>
  );
}
    ...
    return (
      <div className="flex h-[520px] flex-col items-center justify-center rounded-2xl border border-dashed border-violet-500/25 bg-black/20 text-center">
        ...
        <p className="mt-2 max-w-sm text-sm text-zinc-400">
          Add past searches to this case to visualize connections and intel nodes.
        </p>
      </div>
    );
  }
