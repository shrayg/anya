...
// Vendored from React Bits Prism (https://reactbits.dev/backgrounds/prism)
// Source: github.com/DavidHDev/react-bits — TS + ogl WebGL shader.
...
const Prism = ({
  ...
  useEffect(() => {
    ...
    // Cap DPR like other site WebGL bgs — Prism's raymarch is heavy.
    ...
    const render = (t: number) => {
      ...
      if (continueRAF && onScreen && tabVisible) {
        raf = requestAnimationFrame(render);
      } else {
        raf = 0;
      }
  ...
  return (
    <div
      className={`prism-container${className ? ` ${className}` : ""}`}
      ref={containerRef}
    />
  );
};
