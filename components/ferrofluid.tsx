...
const prepColors = (input?: string[]) => {
  // Anya ice-blue defaults (not React Bits demo purple)
  const base = (input && input.length ? input : ["#c3d3e6", "#8fa8c4", "#d8e6f4"]).slice(0, MAX_COLORS);
...
const Ferrofluid: React.FC<FerrofluidProps> = ({
  ...
  useEffect(() => {
    ...
    // Window listener so parent `pointer-events-none` still tracks the cursor
    // without blocking hero / CTA clicks.
    if (mouseInteraction) {
