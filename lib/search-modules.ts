...
> = {
  ...
  /** Domain pivots live inside Stealer Logs (Domain field / auto-detect). */
  domain: { slug: "stealer-logs" },
  domains: { slug: "stealer-logs" },
...
export function isOathnetApiType(apiType: string | null | undefined): boolean {
  if (!apiType) return false;

  return apiType === "oathnet" || apiType.startsWith("oathnet/");
}
...
export type SearchModuleDef = {
  ...
  /**
   * Force the field-type dropdown even when types are auto-detectable
   * (e.g. Stealer Logs: pick Domain / Email / IP, or let auto-detect win).
   */
  forceFieldTypePicker?: boolean;
  /**
...
export const SEARCH_MODULE_SECTIONS: SearchModuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      mod(
        ...
        "email-presence",
        "Email or phone…",
        "Requires a valid email or phone number. Checks account presence across commerce and media sites (included). Toggle Deep search (+1 credit) for Instagram, Snapchat, TikTok, Facebook, Discord, LinkedIn signup, and adult platforms.",
      ),
      mod(
        "Identity",
        "Index Sweep",
        ...
        "index-sweep",
        "Email or phone…",
        'Requires a valid email or phone number. Runs strict quoted "id" site: searches across LinkedIn, GitHub, and other public indexed platforms. Loose leads stay low confidence unless corroborated.',
      ),
      mod(
        "Identity",
        "Phone Index",
  ...
  },
  {
    ...
    items: [
      ...
      // Legacy — hidden from hub; redirects to Stealer Logs (Domain field / auto-detect).
      mod(
        "Network",
        "Domain",
        "domain",
        ...
        "Merged into Stealer Logs — select Domain or paste example.com to auto-detect; runs stealer + breach/domain pivots.",
        undefined,
        undefined,
        {
          tools: [
