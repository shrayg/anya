...
export async function searchCourtListener(
  ...
  if (!token) {
    throw new Error("CourtListener is not configured (missing COURTLISTENER_TOKEN).");
  }
  ...
  const hits: CourtCaseHit[] = (data.results ?? []).slice(0, limit).map((row, index) => {
    ...
    return {
      id: String(row.docket_id ?? row.cluster_id ?? row.id ?? `${q}-${index}`),
      ...
      snippet: row.snippet?.replace(/<\/?[^>]+(>|$)/g, "").trim() || undefined,
      source: {
        id: "courtlistener",
...
export async function probeCourtListener(): Promise<boolean> {
  ...
  try {
    await searchCourtListener({ raw: "smith", mode: "raw", fullName: "Smith" }, 1);
    return true;
  } catch {
    return false;
  }
