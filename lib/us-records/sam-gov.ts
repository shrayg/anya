...
export async function searchSamExclusions(
  ...
  const hits: PersonHit[] = (data.excludedEntity ?? [])
    .slice(0, limit)
    .map((row, index) => ({
      id: `sam-${row.ueiSAM || row.cageCode || index}`,
      name: row.legalBusinessName || q,
      kind: "business",
      subtitle: [row.exclusionProgram, row.exclusionType]
        .filter(Boolean)
        .join(" · "),
      country: "US",
      details: [
        ...(row.excludingAgencyCode
          ...
          : []),
        ...(row.activationDate
          ? [{ label: "Activation", value: row.activationDate }]
          : []),
        ...(row.terminationDate
          ? [{ label: "Termination", value: row.terminationDate }]
          : []),
