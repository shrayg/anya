...
function mergeCookies(existing: string, setCookies: string[]): string {
  ...
  for (const raw of setCookies) {
    ...
    if (raw.toLowerCase().includes("expires=thu, 01 jan 1970") || value === "") {
      map.delete(name);
    } else {
      map.set(name, value);
    }
...
export async function searchVaSexOffenderRegistry(
  ...
  if (!token) {
    throw new Error("Virginia Sex Offender Registry antiforgery token unavailable.");
  }
  ...
  const hits: PersonHit[] = (data.offenders ?? []).slice(0, limit).map((row, index) => {
    ...
    return {
      ...
      name,
      kind: "sex-offender",
      subtitle: [row.addressType, row.age != null ? `Age ${row.age}` : null]
        .filter(Boolean)
        .join(" · ") || undefined,
      state: "VA",
      details: [
        ...(row.age != null ? [{ label: "Age", value: String(row.age) }] : []),
        ...(row.addressType ? [{ label: "Address type", value: row.addressType }] : []),
        ...(location ? [{ label: "Registered location", value: location }] : []),
        ...(row.county ? [{ label: "County / city", value: row.county }] : []),
