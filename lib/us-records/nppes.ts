...
export async function searchNppes(
  ...
  if (
    ...
    data = await fetchUsRecordsJson<NppesResponse>(
      `${NPPES_BASE}?${fallback}`,
      {
        source: "nppes",
        minIntervalMs: 200,
  ...
  const hits: PersonHit[] = (data.results ?? [])
    ...
    .map((row, index) => {
      ...
      return {
        ...
        kind: "provider",
        subtitle:
          [primaryTaxonomy, basic.credential].filter(Boolean).join(" · ") ||
          undefined,
        ...
        details: [
          ...
          ...(primaryTaxonomy
            ? [{ label: "Taxonomy", value: primaryTaxonomy }]
            : []),
