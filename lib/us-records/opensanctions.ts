...
export async function searchOpenSanctions(
  ...
  if (!key) {
    throw new Error(
      "OpenSanctions is not configured (missing OPENSANCTIONS_API_KEY).",
    );
  }
  ...
  const hits: PersonHit[] = (data.results ?? [])
    ...
    .map((entity) => {
      ...
      const nationality =
        prop(entity, "nationality") || prop(entity, "country");
      ...
      return {
        ...
        kind: "sanctions",
        subtitle:
          [entity.schema, datasets].filter(Boolean).join(" · ") ||
          "Global sanctions entity",
        ...
        details: [
          ...
          ...(entity.last_seen
            ? [{ label: "Last seen", value: entity.last_seen }]
            : []),
        ],
        source: {
          id: "opensanctions",
